...
export function LabelAnimation() {
  ...
  return (
    <section className="relative py-20 overflow-hidden bg-gradient-to-b from-neutral-50 to-white">
      ...
      <div className="relative mt-8 sm:mt-12 overflow-hidden">
        <div className="flex animate-scroll">
          {consumables.length > 0 && [...consumables, ...consumables].map((item, index) => (
            ...
            >
              <div className="w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 bg-white rounded-lg shadow-md p-2 sm:p-3 border border-neutral-100 relative">
                <OptimizedImage
                  src={getImageUrl(item.imageUrl)}
                  alt={item.name}
                  className="w-full h-full object-contain"
      ...
      <div className="hidden sm:block absolute top-10 left-10 w-20 h-20 border-2 border-red-200 rounded-full animate-pulse opacity-50"></div>
      <div className="hidden sm:block absolute bottom-20 right-10 w-16 h-16 border-2 border-red-300 rounded-full animate-pulse opacity-50"></div>
      <div className="hidden sm:block absolute top-1/2 left-5 w-3 h-3 bg-red-400 rounded-full animate-bounce"></div>
      <div className="hidden sm:block absolute top-1/3 right-20 w-2 h-2 bg-red-500 rounded-full animate-bounce delay-300"></div>
    </section>
  );
}
